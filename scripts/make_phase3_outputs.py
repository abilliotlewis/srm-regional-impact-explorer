"""Create Phase 3 native-grid regional tables and publication figures."""
...
def calculate(frame: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for season in SEASONS:
        for scenario, comparison in (
            ...
            values = regional_differences(frame, scenario, "tasmax_mean", season)
                ...
                rows.append(
                    {
                        ...
                        "reference_variant_label": frame.loc[
                            (frame.model == record["model"]) & (frame.scenario == "ssp585"),
                            "variant_label",
                        ].iloc[0],
                        "grid_label": record["grid_label"],
                        "value_degC": record["value"],
                    }
                )
        values = intervention_differences(frame, "tasmax_mean", season)
        for record in values.to_dict("records"):
            rows.append(
                {
                    "model": record["model"],
    ...
    stats_input = per_model.rename(
        columns={"comparison": "scenario", "value_degC": "value"}
    )
...
def make_jja_figure(per_model: pd.DataFrame, ensemble: pd.DataFrame, output: Path) -> None:
    ...
    for offset, comparison in zip(offsets, COLORS, strict=True):
        ...
        y = np.array([values.loc[model, "value_degC"] for model in models])
        axes[0].scatter(x + offset, y, s=54, color=COLORS[comparison], label=comparison, zorder=3)
        summary = ensemble[(ensemble.season == "JJA") & (ensemble.comparison == comparison)].iloc[0]
        axes[0].errorbar(
            len(models) + offset,
            summary["mean"],
            yerr=summary["std"],
    ...
    axes[0].set_xticks(np.arange(len(models) + 1), models + ["Ensemble\nmean ± 1 SD"], rotation=18, ha="right")
    axes[0].set_ylabel("Regional tasmax response (°C)")
    ...
    y = np.array([difference.loc[model, "value_degC"] for model in models])
    ...
    axes[1].set_xticks(np.arange(len(models) + 1), models + ["Ensemble\nmean ± 1 SD"], rotation=22, ha="right")
    axes[1].set_ylabel("G6solar minus G6sulfur (°C)")
    axes[1].set_title("Intervention difference")
    axes[1].grid(axis="y", alpha=0.22)

    fig.suptitle(
        "Southeast U.S. JJA tasmax response, 2071-2100\n"
...
def make_seasonal_figure(per_model: pd.DataFrame, ensemble: pd.DataFrame, output: Path) -> None:
    comparison = "G6solar - G6sulfur"
    selected = per_model[per_model.comparison == comparison]
    ...
    for model, values in selected.groupby("model"):
        ...
        ax.plot(SEASONS, ordered.value_degC, color="#8c8c8c", alpha=0.65, linewidth=1.2)
        ax.scatter(SEASONS, ordered.value_degC, color="#8c8c8c", alpha=0.8, s=20)
    summary = ensemble[ensemble.comparison == comparison].set_index("season").loc[SEASONS]
    ax.fill_between(SEASONS, summary.minimum, summary.maximum, color="#8073ac", alpha=0.18, label="Model range")
    ax.plot(SEASONS, summary["mean"], color="#542788", marker="D", linewidth=2.4, label="Multimodel mean")
    ax.axhline(0, color="#333333", linewidth=0.9)
    ax.set(
        ylabel="G6solar minus G6sulfur response (°C)",
...
def main() -> None:
    ...
    parser.add_argument("--data", type=Path, default=ROOT / "data/processed/regional_metrics.csv")
    parser.add_argument("--docs", type=Path, default=ROOT / "docs")
    args = parser.parse_args()
    frame = load_metrics(args.data)
    per_model, ensemble = calculate(frame)
    per_model.to_csv(args.docs / "phase3_per_model.csv", index=False, float_format="%.6f")
    ensemble.to_csv(args.docs / "phase3_ensemble_summary.csv", index=False, float_format="%.6f")
    make_jja_figure(per_model, ensemble, args.docs / "phase3_tasmax_ensemble.png")
    make_seasonal_figure(per_model, ensemble, args.docs / "phase3_tasmax_seasonal.png")
